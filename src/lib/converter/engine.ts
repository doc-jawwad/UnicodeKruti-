...
/** Public consonant table rows derived from KDC_MAP (cannot drift from the engine). */
const PUBLIC_CONSONANTS: { unicode: string; name: string }[] = [
  { unicode: 'क', name: 'ka' },
  { unicode: 'ख', name: 'kha' },
  { unicode: 'ग', name: 'ga' },
  { unicode: 'घ', name: 'gha' },
  ...
  { unicode: 'स', name: 'sa' },
  { unicode: 'ह', name: 'ha' },
  { unicode: 'ड', name: 'da (retroflex)' },
];

export function getPublicConsonantRows(): [string, string, string, string][] {
  const inverse = new Map<string, string>();
  Object.keys(KDC_MAP)
    .sort((a, b) => b.length - a.length)
    .forEach((key) => {
      const val = KDC_MAP[key];
      if (!inverse.has(val)) inverse.set(val, key);
    });

  return PUBLIC_CONSONANTS.map(({ unicode, name }) => {
    const kd = inverse.get(unicode) ?? '?';
    const cp = unicode.codePointAt(0)!;
    const uni = `U+${cp.toString(16).toUpperCase().padStart(4, '0')}`;
    return [unicode, name, kd, uni];
  });
}